...
(function (global) {
  ...
  function renderVerticalTimelineHtml(rows, opts) {
    ...
    return rows.map(function (item, idx) {
      ...
      return '' +
        '<div style="display:grid;grid-template-columns:18px minmax(0,1fr);gap:10px;align-items:start;position:relative;">' +
          '<div style="position:relative;min-height:' + (isLast ? "22px" : "82px") + ';">' +
            '<span style="position:absolute;left:50%;top:6px;transform:translateX(-50%);width:14px;height:14px;border-radius:50%;background:' + color + ';box-shadow:0 0 0 7px ' + color + '18;"></span>' +
          ...
          '<div style="padding:0 0 ' + (isLast ? "0" : "22px") + ';min-width:0;">' +
            '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;min-width:0;">' +
              ...
              (status ? '<span style="display:inline-flex;align-items:center;justify-content:center;padding:5px 12px;border-radius:6px;font-size:13px;font-weight:700;background:' + color + '18;color:' + color + ';white-space:nowrap;flex:none;">' + esc(status) + '</span>' : '') +
            '</div>' +
  ...
  var SIDEBAR_ACTION_HREF = {
    ...
    "proj-company-plan-manage": "purchase-plan-management.html?pageSub=项目公司采购信息台帐",
    "proj-company-contract-mgmt": "contract-management.html?v=20260701-product-code-seq&pageSub=项目公司合同信息管理",
    "proj-company-summary-report": "purchase-summary-report.html?pageSub=项目公司采购合同报表管理",
    "purchase-contract-mgmt": "contract-management.html",
